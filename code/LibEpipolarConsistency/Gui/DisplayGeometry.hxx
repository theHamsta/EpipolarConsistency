...
namespace EpipolarConsistency
{
	...
	struct BoundingBox : public GetSetGui::Configurable {
		struct Style {
			...
			Eigen::Vector4d color = Eigen::Vector4d( 0, 0, 1, 0.5 );
			int             line_width = 2;
		} style;
		struct Corners {
		...
		{
			// Line and Face Style
			GetSet<Eigen::Vector4d> ( "Style/Color", section, style.color      ).setDescription( "" );
		...
		{
			// Line and Face Style
			style.color = GetSet<Eigen::Vector4d>      ( "Style/Color", section );
		...
		{
			Eigen::Vector4d c( 255 * style.color );
